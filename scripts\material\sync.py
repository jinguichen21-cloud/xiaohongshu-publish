...
def remove_directory(
    ...
    Args:
        directory: 要移除的目录路径。
        remove_from_db: 是否同时从向量数据库中删除该目录下的素材。
        config: 素材管理配置。
    ...
    if remove_from_db:
        try:
            ...
            for i, material_id in enumerate(all_materials.get("ids", [])):
                meta = (all_materials.get("metadatas") or [])[i] if i < len(all_materials.get("metadatas", [])) else {}
                file_path = meta.get("file_path", "")
                if file_path.startswith(directory + os.sep) or file_path.startswith(directory + "/"):
